...
const renderProductPage = () => {
    ...
    form.addEventListener('submit', (e) => {
        ...
        const newProduct = {
            ...
            mrp: parseInt(document.getElementById('product-mrp').value, 10),
            stockMM: parseInt(document.getElementById('product-stock-mm').value, 10),
            stockMax: parseInt(document.getElementById('product-stock-max').value, 10)
        };
...
const addSale = (e) => {
    ...
    if (!appState.salesLedger[monthKey]) { showMessage("Please select a valid month log."); return; }
    ...
    if (product[stockField] <= 0) {
        showMessage(`Out of stock for ${product.model} at ${shop} Mobiles.`);
        return;
    }
    ...
    const newSale = {
        ...
        shop: shop,
        mrp: parseInt(formData.get('mrp'), 10),
...
const deleteSale = (saleId, monthKey) => {
    ...
    if (product) {
        const stockField = `stock${saleToDelete.shop}`;
        product[stockField]++;
    }
...
const renderSalesTable = () => {
    ...
    salesToDisplay.sort((a,b) => new Date(b.date) - new Date(a.date)).forEach((sale, index) => {
        const row = document.createElement('tr');
        row.innerHTML = `
